                          ...
                          size_t *out_count)
{
    *out_count = 0;
    ...
    if (s->len + data_len > AWP_STREAM_BUF_SIZE) {
        ESP_LOGW(TAG, "Stream buffer overflow: %zu + %zu > %d",
                 s->len, data_len, AWP_STREAM_BUF_SIZE);
        /* Drop oldest data to make room */
        if (data_len >= AWP_STREAM_BUF_SIZE) {
            /* New data alone exceeds buffer — reset and take tail */
            memcpy(s->buf, data + data_len - AWP_STREAM_BUF_SIZE, AWP_STREAM_BUF_SIZE);
            s->len = AWP_STREAM_BUF_SIZE;
        } else {
            size_t shift = (s->len + data_len) - AWP_STREAM_BUF_SIZE;
    ...
    while (s->len >= AWP_MIN_FRAME_SIZE && *out_count < max_frames) {
        ...
        /* Sanity check */
        if (frame_len < AWP_MIN_FRAME_SIZE || frame_len > AWP_STREAM_BUF_SIZE) {
